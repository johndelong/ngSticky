	...
	module.directive('sticky', function() {
		...
		function linkFn($scope, $elem, $attrs) {
			...
			$scope.$watch( function() { // triggered on load and on digest cycle
				...
				if ( newVal !== oldVal || typeof stickyTopLine === 'undefined' ) {
					stickyTopLine = newVal - offset;
          ...
          var parentHeight = parseInt(parent.offsetHeight);
                    ...
                    if (confine) {
                        $elem.parent().css({
							...
							stickyTopLine = _getTopOffset(parent) - offset;
                    }
					...
					var viewportTop = _getTopOffset(elem);

                    // specify the bottom out line for the sticky to unstick
                    stickyBottomLine = viewportTop + parentHeight - (elem.offsetTop + elem.offsetHeight) - offset;
